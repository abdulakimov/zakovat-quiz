...
export async function POST(request: Request) {
  ...
  if (!teamId) return jsonError("Team is required.");
  ...
  if (team.ownerId !== session.sub) return jsonError("Forbidden", 403);
  ...
  const ext = safeExtFromFile(file.name || "team-avatar", file.type);
  const fileName = `${randomUUID()}${ext}`;
  const relativePath = path.posix.join("team-avatars", teamId, fileName);
  const absoluteDir = path.resolve(UPLOAD_ROOT, "team-avatars", teamId);
  ...
  const asset = await prisma.mediaAsset.create({
    data: {
      ...
      type: "IMAGE",
      path: relativePath,
      originalName: file.name || "team-avatar",
      size: file.size,
      mimeType: file.type,
      sizeBytes: file.size,
