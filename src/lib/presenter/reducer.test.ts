...
import { presenterReducer, type PresenterContext, type PresenterState, type PresenterItem } from "./reducer";
...
function createContext(items: PresenterItem[], questionTimers: Record<string, number>, writeDurationSec = 60): PresenterContext {
  return {
    items,
    writeDurationSec,
    getQuestionTimerSec: (id) => questionTimers[id] ?? 60,
  };
}
...
export function runPresenterReducerTests() {
  runTest("T1 ASK_TIMER behavior", () => {
    ...
    let state = presenterReducer(initialState(), { type: "SET_INDEX", index: 0 }, ctx);

    state = presenterReducer(state, { type: "NEXT", nowMs: 1000 }, ctx);
    ...
    state = presenterReducer(state, { type: "NEXT", nowMs: 1200 }, ctx);
    ...
    state = presenterReducer(state, { type: "SET_INDEX", index: 0 }, ctx);
    state = presenterReducer(state, { type: "NEXT", nowMs: 2000 }, ctx);
    state = presenterReducer(state, { type: "TIMER_TICK", nowMs: 2000 + 30000 }, ctx);
    assert(state.timerStatus === "FINISHED", "Timer should finish");
    state = presenterReducer(state, { type: "NEXT", nowMs: 23000 }, ctx);
  ...
  runTest("T3 Non-timer slides advance immediately", () => {
    ...
    const ctx = createContext(items, { q1: 30 });
    let state = presenterReducer(initialState(), { type: "SET_INDEX", index: 0 }, ctx);
    state = presenterReducer(state, { type: "NEXT", nowMs: 0 }, ctx);
    assert(state.index === 1, "ASK_READ should advance immediately");
  });

  runTest("T4 Timer does not restart without NEXT", () => {
    const items: PresenterItem[] = [{ kind: "ASK_TIMER", roundId: "r1", questionId: "q1" }];
    const ctx = createContext(items, { q1: 10 });
    let state = presenterReducer(initialState(), { type: "SET_INDEX", index: 0 }, ctx);
    state = presenterReducer(state, { type: "NEXT", nowMs: 0 }, ctx);
    assert(state.timerStatus === "RUNNING", "Timer should start");
    state = presenterReducer(state, { type: "TIMER_TICK", nowMs: 2000 }, ctx);
    assert(state.timerStatus === "RUNNING", "Timer should keep running");
    state = presenterReducer(state, { type: "TIMER_TICK", nowMs: 10000 }, ctx);
    assert(state.timerStatus === "FINISHED", "Timer should finish");
    state = presenterReducer(state, { type: "TIMER_TICK", nowMs: 12000 }, ctx);
