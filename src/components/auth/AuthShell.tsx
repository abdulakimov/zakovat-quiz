      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-5xl items-start px-4 py-8 lg:items-center">
        <div className="w-full rounded-3xl border border-border bg-card p-4 shadow-sm md:p-6 dark:shadow-none">
          <div className="grid gap-6 md:grid-cols-2 md:items-stretch">
            <section
              className="rounded-2xl border border-border/80 bg-card p-6 md:p-8"
              data-testid="auth-left"
            >
            ...
            <section
              className="hidden rounded-2xl border border-border/70 bg-gradient-to-br from-primary/10 via-background to-secondary/10 p-4 md:flex md:items-center md:justify-center md:p-6"
              data-testid="auth-right"
            >
              {rightContent ?? (
                  ...
                  <div className="mx-auto grid h-28 w-28 grid-cols-8 gap-1 rounded-xl border border-border bg-background p-2 sm:h-32 sm:w-32 lg:h-40 lg:w-40">
                    {Array.from({ length: 64 }).map((_, idx) => {
                      ...
                      return (
                        <span
                          key={idx}
                          className={darkCell ? "rounded-[2px] bg-foreground/80" : "rounded-[2px] bg-transparent"}
                        />
                      );
                    })}
                  </div>
